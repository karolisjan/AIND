...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """      
        # Each A level is a set of PgNode_a nodes
        self.a_levels.append(set())
        # Iterate through all possible actions for the problem
        for action in self.all_actions:
            # If all prerequisite literals for the action hold in a previous S level
            # add a node for the corresponding action to the current A level. 
            ...
            for pg_node_s in self.s_levels[level]:
                if pg_node_s in pg_node_a.prenodes:
                    # Once an action node is added it MUST be connected to the 
    ...
    def add_literal_level(self, level):
        ...
        """
        # Each S level is a set of PgNode_s nodes
        self.s_levels.append(set())
        # Every A node in the previous level has a list of S nodes in effnodes that
        # represent the effect produced by the action. These literals will be
        # part of the new S level. Since we are working with sets, they
        ...
        for pg_node_a in self.a_levels[level - 1]:
            for eff_node in pg_node_a.effnodes:
                # It is important to then correctly create and connect
                # all of the new S nodes as children of all the A nodes
                # that could produce them, and likewise add the A nodes to the
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Loop through the preconditions of the two nodes
        for parent_of_node_a1 in node_a1.parents:
            for parent_of_node_a2 in node_a2.parents:
                # If one of the precondition of one action is mutex with a
                # precondition of the other action return True
                if parent_of_node_a1.is_mutex(parent_of_node_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Loop through the actions that achieve the two effects
        for parent_of_node_s1 in node_s1.parents:
            for parent_of_node_s2 in node_s2.parents:
                if not parent_of_node_s1.is_mutex(parent_of_node_s2):
                    return False       
        # There must be no actions that could achieve the two effects 
        # at the same time
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        # Loop through the goal states
        for goal in self.problem.goal:
            goal_node = PgNode_s(goal, True)
            # Loop through the levels in the planning graph
            for level in range(len(self.s_levels)):
                # Add the level cost, i.e. level number, where the node 
                # corresponding to the goal is present for the first time
